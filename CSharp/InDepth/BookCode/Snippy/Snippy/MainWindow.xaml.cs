...
namespace Snippy
{
    ...
    public partial class MainWindow : Window
    {
        ...
        async void CompileAndRun(object sender, RoutedEventArgs e)
        {
            Snippet snippet = new Snippet(codeBox.Text, CurrentOptions);
            ...
            else
            {
                ...
                await Execute(snippetType);
            }
        }

        private async Task Execute(Type snippetType)
        {
            // Run the action in a new thread, so it's able to handle some of the examples.
            Task task = Task.Run(() => snippetType.GetMethod("Main").Invoke(null, new object[] { new string[] { } }));
            try
            {
                await task;
            }
